...
import { TOKENS, PINNED_DOMAIN_SEPARATOR, REGISTRY, EIP1967_IMPL_SLOT, PINNED_IMPL } from './constants.ts'
...
/** The proxies whose implementation is worth watching: the two registries and the four tokens. */
const PROXY_TARGETS = [
  { key: 'identity', label: 'ERC-8004 Identity Registry', address: REGISTRY.identity },
  { key: 'reputation', label: 'ERC-8004 Reputation Registry', address: REGISTRY.reputation },
  { key: 'U', label: 'U (United Stables)', address: TOKENS.U.address },
  { key: 'USD1', label: 'USD1 (World Liberty Financial USD)', address: TOKENS.USD1.address },
  { key: 'USDT', label: 'USDT (BSC-USD)', address: TOKENS.USDT.address },
  { key: 'USDC', label: 'USDC (Binance-Peg)', address: TOKENS.USDC.address },
] as const
...
export interface ProxyFingerprint {
  ...
  label: string
  address: string
  /** False when the slot reads empty, meaning the address is not an EIP-1967 proxy. */
  isProxy: boolean
  impl: string | null
  implPinned: string | null
  matchesPinned: boolean | null
  error: string | null
}

export interface ProxyFingerprints {
  block: number | null
  readAt: number
  proxies: ProxyFingerprint[]
}

/** The EIP-1967 implementation behind each registry and token, read live and matched to the pin. */
export function proxyFingerprints(): Promise<ProxyFingerprints> {
  return cached('stack.proxies', CHAIN_READ_CEILING_S, loadProxyFingerprints)
}

async function loadProxyFingerprints(): Promise<ProxyFingerprints> {
  try {
    return await withRpc(async (c) => {
      const block = await c.getBlockNumber()
      const raws = await Promise.all(
        PROXY_TARGETS.map((t) =>
          c
            .getStorageAt({ address: t.address as Hex, slot: EIP1967_IMPL_SLOT, blockNumber: block })
            .then((v) => v ?? null)
            .catch(() => null),
        ),
      )
      const proxies = PROXY_TARGETS.map((t, i): ProxyFingerprint => {
        const pinned = PINNED_IMPL[t.key] ?? null
        const raw = raws[i]
        if (!raw) {
          return { key: t.key, label: t.label, address: getAddress(t.address), isProxy: false, impl: null, implPinned: pinned, matchesPinned: null, error: 'slot read returned nothing' }
        }
        const isProxy = raw.toLowerCase() !== ZERO_SLOT
        const impl = isProxy ? getAddress(`0x${raw.slice(-40)}`) : null
        const matchesPinned = pinned === null || impl === null ? null : impl.toLowerCase() === pinned.toLowerCase()
        return { key: t.key, label: t.label, address: getAddress(t.address), isProxy, impl, implPinned: pinned, matchesPinned, error: null }
      })
      return { block: Number(block), readAt: Date.now(), proxies }
    })
  } catch (e) {
    const msg = (e instanceof Error ? e.message : String(e)).slice(0, 160)
    return {
      block: null,
      readAt: Date.now(),
      proxies: PROXY_TARGETS.map((t): ProxyFingerprint => ({
        key: t.key,
        label: t.label,
        address: getAddress(t.address),
        isProxy: false,
        impl: null,
        implPinned: PINNED_IMPL[t.key] ?? null,
        matchesPinned: null,
        error: msg,
      })),
    }
